...
{{ id }} = new Highcharts.Chart(
        ...
        series: [{%for key, line in graph.cached_data %}{name: '{{ line.label }}', data: [{% for pid, lvalue in line.values.items|sorted %}{% if graph.options.only_percent %}{{ lvalue.percent|default_if_none:"null"|percentraw:1 }}{% else %}{{ lvalue.value|default_if_none:"null" }}{% endif %},{% endfor %}]},{% endfor %}],
