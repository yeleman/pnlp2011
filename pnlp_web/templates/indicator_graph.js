{% load babel %}{% load pnlp %}PERCENT = {% if graph.options.only_percent %}'%'{% else %}''{% endif %};
{{ id}} = new Highcharts.Chart(
        ...
        series: [{%for key, line in graph.data %}{name: '{{ line.label }}', data: [{% for pid, lvalue in line.values.items %}{% if graph.options.only_percent %}{{ lvalue.percent|default:"0"|percentraw }}{% else %}{{ lvalue.value|default_if_none:"-0" }}{% endif %},{% endfor %}]},{% endfor %}],
