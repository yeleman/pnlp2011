...
def report_as_excel(report):
    ...
    sheet.write_merge(6, 6, 0, 1, u"Total consultation, toutes causes confondues", stylelabel)
    sheet.write_merge(7, 7, 0, 1, u"Nbre de Cas de paludisme (Tous suspectés)", stylelabel)
    sheet.write_merge(8, 8, 0, 1, u"Cas de paludisme testés (GE et/ou TDR)", stylelabel)
    sheet.write_merge(9, 9, 0, 1, u"Cas de paludisme confirmés (GE et/ou TDR)", stylelabel)
    sheet.write_merge(10, 10, 0, 1, u"Nbre de Cas de paludisme Simple", stylelabel)
    sheet.write_merge(11, 11, 0, 1, u"Nbre de Cas de paludisme Grave", stylelabel)
    sheet.write_merge(12, 12, 0, 1, u"Nbre de Cas traités avec CTA", stylelabel)
    ...
    sheet.write_merge(17, 17, 0, 1, u"Total Hospitalisés Paludisme", stylelabel)
    ...
    sheet.write_merge(22, 22, 0, 1, u"Cas de décès pour paludisme", stylelabel)
    ...
    sheet.write_merge(8, 8, 9, 12, u"PEC de cas de Paludisme grave", styletitle)
    sheet.write_merge(9, 9, 9, 12, u"Rupture de soctk OUI/NON", styletitle)
    ...
    sheet.write_merge(14, 14, 10, 12, u"Rupture de stock pendant le mois O/N \n(Oui, Non)", styletitle)
    ...
    sheet.write_merge(19, 20, 10, 12, u"CPN/SP des femmes enceintes (nbre)", styletitleform)
        ...
        for source in report.sources():
            source_str = u"%(entity)s - %(receipt)s / %(level)s" \
                         % {'entity': source.entity.display_name(),
                            ...
                            'level':
                            u"PRIMAIRE" if source.REPORTING_LEVEL == SOURCE_LEVEL else u"AGGRÉGÉ"}
